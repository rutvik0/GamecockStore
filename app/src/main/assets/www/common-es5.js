  ...
  function srcAppItemServiceTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var ItemService = /*#__PURE__*/function () {
      function ItemService(router) {
        ...
        this.cart = [];
        var self = this;
        this.db.collection("CartOrder").onSnapshot(function (querySnapshot) {
          self.orders = [];
        ...
        this.db.collection("items").onSnapshot(function (querySnapshot) {
          console.log("items list changed...");
          self.items = [];
          querySnapshot.forEach(function (doc) {
            var item = doc.data(); // console.log(doc.id)
        ...
        this.db.collection("cart").onSnapshot(function (querySnapshot) {
          console.log("cart changed...");
          self.cart = [];
          querySnapshot.forEach(function (doc) {
            var cart = doc.data(); // console.log(doc.id)
      ...
      _createClass(ItemService, [{
        ...
        value: function setUsertype(type) {
          var self = this;
          this.db.collection("CartOrder").where("uid", "==", firebase__WEBPACK_IMPORTED_MODULE_4__["auth"]().currentUser.uid).onSnapshot(function (querySnapshot) {
            self.orders = [];
            querySnapshot.forEach(function (doc) {
              var test = doc.data();
              self.orders.push({
          ...
          if (this.usertype == "visitor") {
            ...
            this.db.collection("items").where("uid", "==", firebase__WEBPACK_IMPORTED_MODULE_4__["auth"]().currentUser.uid).onSnapshot(function (querySnapshot) {
              console.log("items list changed...........");
              self.items = [];
              querySnapshot.forEach(function (doc) {
                var item = doc.data();
          ...
          this.db.collection("cart").where("uid", "==", firebase__WEBPACK_IMPORTED_MODULE_4__["auth"]().currentUser.uid).onSnapshot(function (querySnapshot) {
            console.log("cart changed...");
            self.cart = [];
            querySnapshot.forEach(function (doc) {
              var cart = doc.data();
        ...
        key: "createItem",
        value: function createItem(title, price, category, image, description) {
          var self = this;
          var uid = null;

          if (firebase__WEBPACK_IMPORTED_MODULE_4__["auth"]().currentUser != null) {
            uid = firebase__WEBPACK_IMPORTED_MODULE_4__["auth"]().currentUser.uid;
            console.log(uid, " :****** uid");
          } else {
            console.log(" no user logged in, no item created");
          }

          var db = firebase__WEBPACK_IMPORTED_MODULE_4__["firestore"]();
          db.collection("items").add({
            ...
            console.log("Document written with ID: ", docRef.id); //update this products arrays
        ...
        value: function deleteOrder(id) {
          var self = this;
          var db = firebase__WEBPACK_IMPORTED_MODULE_4__["firestore"]();
          db.collection("CartOrder").doc(id)["delete"]().then(function () {
            self.router.navigate(["/tabs/tab2"]);
          })["catch"](function (error) {
            console.log("error with removing doc");
          });
        }
      }, {
        key: "deleteItem",
        value: function deleteItem(id) {
          ...
          db.collection("items").doc(id)["delete"]().then(function () {
            ...
            console.error("error with removing doc");
          });
        }
      }, {
        key: "addToCart",
        value: function addToCart(cost, quantity, title, image, uid) {
          var db = firebase__WEBPACK_IMPORTED_MODULE_4__["firestore"]();
        ...
        value: function deleteCart(id) {
          ...
          db.collection("cart").doc(id)["delete"]().then(function () {
            self.router.navigate(["/tabs/tab3"]);
          })["catch"](function (error) {
            console.error("error with removing doc");
          });
        }
        ...
        key: "placeOrder",
        value: function placeOrder(cart, oid, uid, total, date, quantity) {
          var db = firebase__WEBPACK_IMPORTED_MODULE_4__["firestore"]();
          db.collection("CartOrder").add({
            'cart': cart,
            'oid': oid,
            'uid': uid,
            'total': total,
        ...
        key: "updateProduct",
        value: function updateProduct(value, id) {
          var self = this;
          var db = firebase__WEBPACK_IMPORTED_MODULE_4__["firestore"]();
          console.log(value);
          db.collection("items").doc(id).update(value);
        }
      }]);
    ...
    var snapshotToArray = function snapshotToArray(snapshot) {
      ...
      snapshot.forEach(function (childSnapshot) {
        ...
        item.id = childSnapshot.key; // console.log(item);
